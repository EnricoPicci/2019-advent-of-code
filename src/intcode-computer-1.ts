...
type IntcodeComputerResponse = { output: number[]; exitCode: ExitCode };

export function intcodeComputer(progam: number[], outputFunction?: (o: number) => void, log = false) {
    let instructionPointer: number;

    const state = buildState(progam);
    ...
    return (input?: number[]) => {
        ...
        while (!currentInstruction.done) {
            ...
            if (opCode === '99') {
                return { output, exitCode: 'END' } as IntcodeComputerResponse;
            } else if (opCode === '01') {
                // SUM
                ...
                if (inputPointer >= input.length) {
                    instructionPointer = currentInstruction.instructionPointer - numberOfElementsPerInstruction('03');
                    return { output, exitCode: 'WAIT FOR INPUT' } as IntcodeComputerResponse;
                }
                ...
                if (firstParamValue !== 0) {
                    const secondParamValue = getParameterValue(
